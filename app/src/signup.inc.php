<?php 
    if(isset($_POST['submit'])) {
        ...
        if(empty($first) || empty($last) || empty($email) || empty($uid) || empty($pwd)) {
            header("Location: ../../pages.php?page=register&error=error2");
            exit();
        } else {
            if(!preg_match("/^[a-zA-Z]*$/", $first) || !preg_match("/^[a-zA-Z]*$/", $last)) {
                header("Location: ../../pages.php?page=register&error=error3");//invalid
                exit();
            } else {
                if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    header("Location: ../../pages.php?page=register&error=error4");//invalidemail
                    exit();
                } else {
                    ...
                    if($result_check > 0) {
                        header("Location: ../../pages.php?page=register&error=error5");//usertaken
                        exit();
                    } else {
                        ...
                        header("Location:  ../../pages.php?page=login&error=error6");
                        exit();
                    }
                }
            }
        ...
        header("Location: pages.php?page=register&error=error1");
        exit();
    }
?>